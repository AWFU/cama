...
module ProductCatesHelper
  ...
  def list_product(current_product)
    if current_product.galleries.select{ |v| v['type'] == 'ProductPhotoGallery' }.count > 0
      "<div class='pull-left'> #{link_to( image_tag(current_product.galleries.select{ |v| v['type'] == 'ProductPhotoGallery' }.first.attachment.url, class: 'img-responsive' ) , category_product_path(@product_cate, current_product)) }</div>".html_safe
    else
