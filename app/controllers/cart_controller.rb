    ...
    begin
      ...
      respond_to do |format|
        if(@order.save)
          ...
          # ORDER ATTRIBUTES
          @order.update({ order_memo: cookies[:cart_cafe_attrs], delivery_type: recheck_delivery_type(@order), shipping_fee: define_shipping_fee(@order, cart_sum)} )

          crypt = ActiveSupport::MessageEncryptor.new(Cama::Application.config.secret_key_base)
          ...
          @order_items = Cart.check_items_in_cart(cookies[:cart_cama], "for_cart")
          @ship_to = order_params[:ship_to] if order_params[:ship_to].in? ["taiwan","island"]
  ...
  def final_check
    ...
    begin
      if Order.find_by_ordernum(ordernum).nil?
        ...
        # ORDER ATTRIBUTES
        @order.update({ order_memo: cookies[:cart_cafe_attrs], shipping_fee: define_shipping_fee(@order, cart_sum)} )
      end
    rescue
      flash[:notice] = "Oops..."
      reset_everything_about_cart
      ...
      unless @order.nil?
        ...
        if @items_all_alive == false
          ...
          if @items_all_dead == true
            ...
            cookies.delete(:cart_cafe_attrs)
            session[:temp_order] = nil
            ...
            @items_dead.each do |deaditem|
              ...
              @cart_cafe_attrs.each do | attr |
                @cart_cafe_attrs.delete(attr[0]) if attr[1]["stock_id"].to_s == deaditem
              end
              cookies[:cart_cafe_attrs] = @cart_cafe_attrs.to_json
            end
            session[:temp_order] = nil
    ...
    def recheck_delivery_type(order)
      ...
      if order.payment_type == "COD"
        ...
        if order.ship_to == "taiwan" 
          Delivery.where("shipping_condition = 'taiwan' and iscod = 'yes'").first.id
        else
          Delivery.where("shipping_condition = 'island' and iscod = 'yes'").first.id
        end

      else
        
        if order.ship_to == "taiwan"
          Delivery.where("shipping_condition = 'taiwan' and iscod = 'no'").first.id
        else
          Delivery.where("shipping_condition = 'island' and iscod = 'no'").first.id
        end
    ...
    def check_item_are_alive(order) #check item stock exist
      ...
      if order.orderitems.count == 0
        ...
        order.orderitems.each do |item|
          if ProductStock.find_by_id(item.product_stock_id).nil? || ProductStock.find_by_id(item.product_stock_id).product.status == 'disable'
            @items_all_alive = false 
            @items_all_dead = true if order.orderitems.count == 1
        
            @items_dead << item.product_stock_id.to_s
          end
        end
      end
